...
# Not gonna use this function since we already have the data

NAME_BASICS <- read_tsv('name.basics.tsv.gz')

TITLE_BASICS <- read_tsv('title.basics.tsv.gz')

TITLE_EPISODES <- read_tsv('title.episode.tsv.gz')
...
TITLES_AND_NAMES # Because there are too many NULL deathYear values, this question can't really be answered
```
...
HIGHEST_RATED # Unsurprisingly, it's a season 5 episode of Breaking Bad, specifically, the one where Hank gets got
```
...
<a href="https://www.youtube.com/watch?v=YCrngJCkI8I" target="_blank">
  <img src="https://img.youtube.com/vi/YCrngJCkI8I/hqdefault.jpg" alt="Video Preview" style="width: 300px; height: auto;">
</a>
...
<a href="https://www.youtube.com/watch?v=_gBWpWwIBKw" target="_blank">
  <img src="https://img.youtube.com/vi/_gBWpWwIBKw/hqdefault.jpg" alt="Video Preview" style="width: 300px; height: auto;">
</a>
...
# checking to see if we can just do a group by
TITLE_EPISODES |>
  filter(parentTconst == "tt0903747")

TITLE_RATINGS |>
  filter(tconst == "tt0903747")

# Yep, this returned all the BB episodes
...
TWELVE_EP_SERIES # I have no clue what any of these shows are
```
...
MOVIES <- MOVIES |>
  left_join(GENRE_COUNT, by = genres)
...
```{r echo=TRUE, message=TRUE, warning=FALSE}
#3.1: Chooose the top 5-10 movies by my metric to confirm successes
MOVIES |>
  filter(metric==1) |>
  sample_n(10)
