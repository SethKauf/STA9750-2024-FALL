...
USAGE |>
  filter(month == '2024-05-01',
         ...
         Mode == "Heavy Rail") |>
  select(unlinked_passenger_trips) #Answer: 180,458,819 trips
...
USAGE |>
  ...
  slice_head(n=3) # NYC Subway leads the way in revenue miles, followed by NJT Bus and LA Bus
...
USAGE |>
  ...
  arrange(desc(total_travel)) |>
  slice_head(n=3) # NYC Subway leads the way here as well, while NJT Bus is overtaken by NYC's bus system
                  # The distance between the MTA and the next few is quite large
...
USAGE_AND_FINANCIALS |>
  ...
  slice_head(n=1) # MTA New York City Transit -- Heavy Rail
...
USAGE_AND_FINANCIALS |>
  ...
  slice_head(n=1) # Transit Authority of Central Kentucky -- Vanpool
...
USAGE_AND_FINANCIALS |>
  ...
  slice_head(n=1) # North Caroline State University -- Bus
...
USAGE_AND_FINANCIALS |>
  ...
  slice_head(n=1) # Athens-Clarke County Unified Government -- Bus
...
USAGE_AND_FINANCIALS |>
  ...
  slice_head(n=1) # New Mexico Department of Transportation -- Vanpool
