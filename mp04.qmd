...
The AlphaVantage data comes from querying on different symbols at https://www.alphavantage.co/query
...
bobs_expenses <- bobs_expenses |>
  mutate(
    annual_col = (monthly_property_tax * 12) + (monthly_col * 12)
  )
...
for (i in 2:nrow(bobs_expenses)){
  ...
  bobs_expenses$starting_pension[i] = bobs_expenses$ending_pension[i - 1]
  
  bobs_expenses$ending_pension[i] = (bobs_expenses$ending_pension[i] <- bobs_expenses$starting_pension[i] + bobs_expenses$contribution[i] - bobs_expenses$annual_col)
