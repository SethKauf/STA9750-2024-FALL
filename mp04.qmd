...
[The Federal Reserve Bank of St. Louis](https://fred.stlouisfed.org/) which has available information on inflation, wage growth, bonds, and U.S. debt

[Alpha Vantage](https://www.alphavantage.co/) which has available information on the U.S. and International Stock Markets.
...
```{r alpha-key_[3], echo=FALSE, message=FALSE, warning=FALSE}
# Key has been registered previously
alpha_api_key <- Sys.getenv("ALPHA_API_KEY")
print("API Key 1 Loaded")
```

## Task 02: Register for FRED API Key
```{r fred-key_[4], echo=FALSE, message=FALSE, warning=FALSE}
# Key has been registered previously
fred_api_key <- Sys.getenv("FRED_API_KEY")
print("API Key 2 Loaded")
```
...
* [Wage Growth](https://fred.stlouisfed.org/series/FRBATLWGTUMHWG83O)
* [Bond market total returns](https://fred.stlouisfed.org/series/BAMLCC0A4BBBTRIV)
* [Short-term debt returns](https://fred.stlouisfed.org/series/QFRD304INFUSNO)

The AlphaVantage data comes from querying on different symbols through their API

* US Equity Market total returns
  - For this one using [SPY](https://finance.yahoo.com/quote/SPY/) which is the S&P 500 Index gives us a good indicator of how U.S. markets are performing in general, 

* International Equity Market total returns
  - Similarly, [VXUS](https://finance.yahoo.com/quote/VXUS/) or the Vanguard Total International Stock Index can give us an inkling for international markets, though it is limited in data (discussed further below)
...
Note: This data spans from January 1983 through October 2024. However, there are two date ranges where there is no recorded data: July 1985 - September 1986 (15 months) and June 1995 - August 1996 (15 months). This is "[...due to changes in methodology.](https://fred.stlouisfed.org/series/FRBATLWGTUMHWG83O)"
...
#| code-fold: TRUE
working_bob_plot <- ggplot(working_bob, aes(x = Year, y = Income)) + # they call him the working bob
  geom_line(color = "#44b80a", size = 1.5) +
...
print(glue("The average salary for Bob's final 3 years of work at CUNY is {dollar(bobs_fas)}"))
```
...
opr_bob <- working_bob |> # I realize now that I've done most of the project writing `opr` instead of `orp`. Although it would probably be best practice to fix this, I won't. Sorry.
  select(
    Year,
    mwg,
    income = Income
...
Bob retired at the end of 2013 at age 70. [According to the CDC](chrome-extension://efaidnbmnnnibpcajpcglclefindmkaj/https://www.cdc.gov/nchs/data/nvsr/nvsr66/nvsr66_03.pdf), as of 2013, the average male-lifespan in the U.S. as of 2013 was 76.4 years. However, let's say that our Bob lives to 80 years, 10 years after he retired.
...
```{r bobs-expenses_[27], echo=TRUE, message=FALSE, warning=FALSE}
#| code-fold: TRUE

# creating dummy variables to run numbers off of
home_purchase_price <- 400000
price_ptax_2014 <- 0.125 # this was the average in 2014
monthly_col_2014 <- 500 # guesstimate, based off of nothing but vibes

bobs_expenses <- retirement_bob |>
  select(
    Year,
    cpi
  )
...
for (i in 2:nrow(bobs_expenses)) {
  bobs_expenses$monthly_property_tax[i] <- (bobs_expenses$monthly_property_tax[i] <- bobs_expenses$monthly_property_tax[i - 1] + (bobs_expenses$monthly_property_tax[i - 1] * bobs_expenses$cpi[i]))

  bobs_expenses$monthly_col[i] <- (bobs_expenses$monthly_col[i] <- bobs_expenses$monthly_col[i - 1] + (bobs_expenses$monthly_col[i - 1] * bobs_expenses$cpi[i]))
}
...
Let's recall the tables `retirement_bob` and `bobs_markets` to look at how far his pension versus withdrawal would've gone in the 10 year span following his retirement.

```{r bob-withdrawls-pension_[28], echo=TRUE, message=FALSE, warning=FALSE}
#| code-fold: TRUE
# bringing in pension

starting_pension_2014 <- retirement_bob$starting_pension[1]


ret_bob <- retirement_bob |>
  select(
    Year,
    contribution
  )

bobs_expenses_trs <- bobs_expenses |>
  left_join(ret_bob, by = "Year") |>
  mutate(
    starting_pension = if_else(row_number() == 1, starting_pension_2014, NA_real_)
  )

bobs_expenses_trs <- bobs_expenses_trs |>
  mutate(
    ending_pension = if_else(row_number() == 1, starting_pension + contribution - annual_col, NA_real_)
  )

for (i in 2:nrow(bobs_expenses_trs)) {
  bobs_expenses_trs$starting_pension[i] <- bobs_expenses_trs$ending_pension[i - 1]

  bobs_expenses_trs$ending_pension[i] <- (bobs_expenses_trs$ending_pension[i] <- bobs_expenses_trs$starting_pension[i] + bobs_expenses_trs$contribution[i] - bobs_expenses_trs$annual_col)
}
...
```{r bob-withdrawls-fund_[29], echo=TRUE, message=FALSE, warning=FALSE}
#| code-fold: TRUE

# looking at bobs expenses w/ orp plan
bobs_expenses_orp <- bobs_expenses |>
  mutate(
    funds = if_else(row_number() == 1, bob_total_savings, NA_real_)
  )

for (i in 2:nrow(bobs_expenses_orp)) {
  bobs_expenses$funds[i] <- (bobs_expenses$funds[i] <- bobs_expenses$funds[i - 1] - bobs_expenses$annual_col[i])
}

bobs_expenses_orp <- bobs_expenses_orp |>
  mutate(
    funds = round(funds, 2)
  )

for (i in 2:nrow(bobs_expenses_orp)) {
  bobs_expenses_orp$funds[i] <- round((bobs_expenses_orp$funds[i - 1] - bobs_expenses$annual_col[i]), 2)
}
...
```{r bootstrap-bob_[30], echo=TRUE, message=FALSE, warning=FALSE}
#| code-fold: TRUE
# Create resampling function
bootstrap_resample <- function(data, n_samples = 200) {
  replicate(n_samples, data[sample(nrow(data), replace = TRUE), ], simplify = FALSE)
}

# resample
trs_bootstrap_samples <- bootstrap_resample(retirement_bob)
orp_bootstrap_samples <- bootstrap_resample(bobs_markets)

# 10 years
simulate_projection <- function(bootstrap_samples, years = 10) {
  projections <- lapply(bootstrap_samples, function(sample) {
    sample |>
      sample_n(years, replace = TRUE) |>
...
trs_projections <- simulate_projection(trs_bootstrap_samples)
orp_projections <- simulate_projection(orp_bootstrap_samples)


projections <- data.frame(
  TRS_Monthly_Pension = trs_projections$monthly_pension,
...
```{r bob-withdrawls-rates-bootstrapped_[31], echo=TRUE, message=FALSE, warning=FALSE}
#| code-fold: TRUE

# simulating projections using bootstrap function with a withdrawl rate of 4% per year for 10 yrs
simulate_projection_with_withdrawals <- function(bootstrap_samples, years = 10, withdrawal_rate = 0.04, type = "TRS") {
  projections <- lapply(bootstrap_samples, function(sample) {
    if (type == "TRS") {
      projected_funds <- sample |>
        sample_n(years, replace = TRUE) |>
        summarise(
      ...
      projected_funds <- sample |>
        ...
        summarise(
          avg_starting_funds = mean(
            bobs_us_shares_price + bobs_intl_shares_price + bobs_bonds_shares_price + bobs_debts_shares_price,
            na.rm = TRUE
          ),
          avg_contributions = 0
        )
    ...
    for (i in 2:years) {
      funds[i] <- funds[i - 1] * (1 - withdrawal_rate)
    }
...
trs_withdrawals <- simulate_projection_with_withdrawals(trs_bootstrap_samples, type = "TRS")
orp_withdrawals <- simulate_projection_with_withdrawals(orp_bootstrap_samples, type = "ORP")

trs_withdrawals$Type <- "TRS"
orp_withdrawals$Type <- "ORP"

withdrawal_projections <- bind_rows(trs_withdrawals, orp_withdrawals)

main_projections <- withdrawal_projections |>
  group_by(Type, Year) |>
  summarise(
